...
public abstract class CodebreakerDatabase extends RoomDatabase {
  ...
  public static class Converters {
    ...
    public static byte[] uuidToBytes(UUID value) {
      byte[] bytes = null;
      if (value != null) {
        ByteBuffer buffer = ByteBuffer.allocate(16); // 16 bytes in a UUID
        buffer.putLong(value.getMostSignificantBits()) // half of the UUID
          .putLong(value.getLeastSignificantBits()); // other half
        bytes = buffer.array(); // take array of values
      }
      return bytes;
    }
