...
public class MatchRepository {
  ...
  public Single<Match> add(Match match) {
    return signInService.refresh() // Gets credentials
    // whatever thread we did refresh on, we want to do it on a thread that we control
        .observeOn(Schedulers.io())
        .flatMap((account) -> webService.startMatch(getBearerToken(account.getIdToken()), match)) // Taking token and turning it into a bearer token
        .flatMap((responseMatch) -> matchDao.insert(responseMatch) // return a long id
            .map((id) -> {
